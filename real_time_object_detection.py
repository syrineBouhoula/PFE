...
filename="executionTimeVideo.csv"
'''csv = open(filename, 'a')	
csv.write("elapsed time for fps,number of fps,number of frames per second\n")
csv.close'''
...
vs = VideoStream("http://192.168.137.237:8160").start()
...
def handler(signum, frame):
    ...
    vs.stop()
    print("average",format(fps.elapsed()))
    print("number of frames",count)
    print("count/elapsedFPS",count/(fps.elapsed()))
    #elapsedfps=int(fps.elapsed())
    frampersecond=int(fps.fps())
    csv = open(filename, 'a')
    csv.write(str(format(fps.elapsed())) + ',')
    csv.write(str(frampersecond)+ '\n')
    csv.close()
    exit(1)
